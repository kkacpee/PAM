...
export default function OngoingTrainingScreen() {
    const [exercises, setExercises] = React.useState<OngoingTrainingViewModel>(items);
    ...
    const state = useAsync(() => controller.GetTodaysTraining(), [useIsFocused()]);
      ...
      <View style={styles.container}>
        <ImageBackground source={require('../assets/images/WorkoutBg.png')} style={styles.image}>  
          <View style={styles.column}>
              ...
              <Text style={styles.content}>Next: {(exercises.length > (currentIndex + 1)) ? exercises[currentIndex + 1].name : 'End'}</Text>
              <Text style={styles.headerToday}>{exercises[currentIndex].name}</Text>
              
            {!end ? (exercises[currentIndex].isTimed ? 
            <TimedWorkout 
            seconds={sec} 
            minutes={min} 
            isActive={isActive}
